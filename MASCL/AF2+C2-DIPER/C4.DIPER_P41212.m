...
parfor r=1:size(Sample,2)
    ...
    % Extract Coordinates of Alpha Carbon (CA)
    HMD=pdbread(['C4.DIPER_Result/',Sample(r).ID, '_pnon.pdb']);
    ...
    Result=fopen(['C4.DIPER_Result/',lower(Sample(r).ID),'/', 'ft.000.00']);
...
save('C4.DIPER_Result/C4_DIPER_Result_P41212.mat', 'Sample');
...
load('C2.DIPER_Result/P41212_C2_TopRank_Result.mat')
...
save('C4.DIPER_Result/C4_TopRank_Result_P41212.mat', 'C4_TopRank_Result');
...
save('C4.DIPER_Result/P41212_TopRank_Result.mat', 'Result');
...
mkdir('C4.DIPER_Result/P41212_TopRank_300_Pose');

% Load C4.DIPER Result
load('C4.DIPER_Result/P41212_TopRank_Result.mat');
load('C2.DIPER_Result/P41212_C2_TopRank_Result.mat');
...
parfor r=1:size(Result,2)
    ...
    % Establish Deposited File
    mkdir(['C4.DIPER_Result/P41212_TopRank_300_Pose/',Result(r).ID]);
    ...
    if ~isempty(Qualified_Idx)
        for QI=1:size(Qualified_Idx,1)
            ...
            HMD=pdbread(['C4.DIPER_Result/',HMD_ID,'_pnon.pdb']);
            HMD_CA_Idx=strcmp({HMD.Model.Atom.AtomName}, 'CA')';
            ...
            Result_C4=fopen(['C4.DIPER_Result/',HMD_ID,'/',set]);
            PIPER_result=zeros(size(Rots_list,1), 10);
            for p=1:size(Rots_list,1)
            ...
            warning('off','all');
            pdbwrite(['C4.DIPER_Result/P41212_TopRank_300_Pose/', Result(r).ID, '/', HMD_ID, set(3:7), num2str(Result_Idx), '.pdb'], Output_SiXt);
            ...
            warning('off','all');
            pdbwrite(['C4.DIPER_Result/P41212_TopRank_300_Pose/', Result(r).ID, '/', HMD_ID, set(3:7), num2str(Result_Idx), '_C2C4.pdb'], Output_C2C4);
            ...
            warning('off','all');
            pdbwrite(['C4.DIPER_Result/P41212_TopRank_300_Pose/', Result(r).ID, '/', HMD_ID, set(3:7), num2str(Result_Idx), '_Ans.pdb'], Output_Xtal);
        end
    end
    r
end
...
save('C4.DIPER_Result/P41212_Sum_Table.mat', 'Sum_Table');