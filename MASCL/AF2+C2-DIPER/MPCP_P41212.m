...
mkdir('MPCP_Result/Packing_Pose');
...
%%% Packing Simulation
% Load MXRGA Result
load('SurfOrca_Result/P41212_MXRGA_Result.mat')
...
for r=1:size(MXRGA,2) % Example: r=21
    ...
    if ~isempty(MXRGA(r).SurfOrca_Cell_Pmt)
        ...
        if exist(['MPCP_Result/P41212_Packing_Pose/',MXRGA(r).PDB_ID],'dir')==0
            mkdir(['MPCP_Result/P41212_Packing_Pose/',MXRGA(r).PDB_ID]);
        end
        ...
        Sim_PDB=pdbread(['SurfOrca_Result/P41212_Refined_TopRank_300_Pose/',MXRGA(r).PDB_ID,'/',MXRGA(r).SiXt_ID,'_Refined.pdb']);
        ...
        try
            ...
            warning('off','all');
            pdbwrite(['MPCP_Result/P41212_Packing_Pose/',MXRGA(r).PDB_ID,'/',MXRGA(r).SiXt_ID, '_Xtal_Packing.pdb'], Xtal_Packing_Output);
            ...
            warning('off','all');
            pdbwrite(['MPCP_Result/P41212_Packing_Pose/',MXRGA(r).PDB_ID,'/',MXRGA(r).SiXt_ID, '_Sim_Packing.pdb'], Packing_Output);
        catch
            Error_ID=[Error_ID; r];
        end
    else
