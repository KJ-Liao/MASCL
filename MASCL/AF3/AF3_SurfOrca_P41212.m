...
mkdir('AF3_SurfOrca_Result/P41212_Refined_TopRank_300_Pose');

% Load TopRank Result
load('AF3_C4.DIPER_Result/P41212_Sum_Table.mat')
load('AF3_Dimer4Docking/P41212_TopRank_Result.mat');
...
for r=1:size(Sum_Table,2) % Example: r=21
    ...
    if exist(['AF3_SurfOrca_Result/P41212_Refined_TopRank_300_Pose/',Sum_Table(r).ID(1:4)],'dir')==0
        mkdir(['AF3_SurfOrca_Result/P41212_Refined_TopRank_300_Pose/',Sum_Table(r).ID(1:4)]);
    end
    ...
    Filename=['AF3_C4.DIPER_Result/P41212_TopRank_300_Pose/',Sum_Table(r).ID(1:4),'/',Sum_Table(r).ID,'.pdb'];
    AF=pdbread(Filename);
    ...
    if ~isempty(Jugle)
        ...
        if ~isempty(Grid_Ocean)
            ...
            warning('off','all');
            pdbwrite(['AF3_SurfOrca_Result/P41212_Refined_TopRank_300_Pose/', MXRGA(r).SiXt_ID(1:4), '/', MXRGA(r).SiXt_ID, '_Refined.pdb'], Output_AF);

            %

            %%%% RMSD 95 Calculation
            % Load Corresponding Ans PDB (Xtal_PDB) for RMSD Calculation
            Filename=['AF3_C4.DIPER_Result/P41212_TopRank_300_Pose/', MXRGA(r).SiXt_ID(1:4), '/', MXRGA(r).SiXt_ID, '_Ans.pdb'];
...
save('AF3_SurfOrca_Result/AF3_P41212_MXRGA_Result.mat','MXRGA');
